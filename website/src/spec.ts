    ...
    render(): any {
        ...
        return <!div>
            {_.map(this.props.ifce.interface, (v: any, k: string) => <!Bootstrap.Accordion key={k} activeKey={this.state.activePropKey} onSelect={this.handlePropSelect}>
                {k === "_extends" ?
                    v.map((a: string, i: string) => <!Bootstrap.Panel key={i} eventKey={a} bsStyle="primary" header={"Extends " + a}>
                            <!div>
                    ...
                    <!Bootstrap.Panel eventKey={k} bsStyle={k.indexOf("<") === 0 ? "success" : "danger"}
                            header={<!span>{!isNaN(v.idx) ? "Property " + v.idx + ". " : ""}
                                {k + " "}
...
class Spec extends TypedReact.Component<Spec.IProps, IState> {
    render(): any {
        ...
        return <!div className="pageContent minipad">
            ...
            <!br /><!br />
            Click on headers to expand them. Good luck!<!br/><!br/>

            <!Bootstrap.Panel key="copyright" eventKey="copyright" header="Copyright">
                <!MultilineInfoBlob.Component spec={spec} prop="copyright" />
            </Bootstrap.Panel>
